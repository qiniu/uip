...
//
// All the methods on Reader are thread-safe. The struct may be safely
...
// speedup for finding metadata
const metaDetectStart = 2000

// FromBytes takes a byte slice corresponding to a aw DB file and returns
// a Reader structure or an error.
func FromBytes(buffer []byte) (*Reader, error) {
	detectOffset := len(buffer) - metaDetectStart
	metaIn := buffer[detectOffset:]
	metadataStart := bytes.LastIndex(metaIn, metadataStartMarker)

	if metadataStart == -1 {
		return nil, newInvalidDatabaseError("error opening database: invalid aw DB file")
	}

	metadataDecoder := decoder{metaIn[metadataStart+len(metadataStartMarker):]}
	...
	metadataStart += detectOffset
	searchTreeSize := metadata.NodeCount * metadata.RecordSize / 4
	dataSectionStart := searchTreeSize + dataSectionSeparatorSize
	dataSectionEnd := uint(metadataStart)
	if dataSectionStart > dataSectionEnd {
		return nil, newInvalidDatabaseError("the aw DB contains invalid metadata")
	}
	d := decoder{
		buffer[searchTreeSize+dataSectionSeparatorSize : metadataStart],
	}
...
func (r *Reader) version() *inf.VersionInfo {
	return &inf.VersionInfo{
		IpType:    r.Metadata.IpType(),
		Count:     uint32(r.Metadata.NodeCount),
		Build:     int64(r.Metadata.BuildEpoch),
		Version:   "ipplus360" + r.Metadata.DatabaseType,
