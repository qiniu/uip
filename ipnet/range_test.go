...
func TestRange2(t *testing.T) {
	ast := assert.New(t)
	_, ipNet0, _ := net.ParseCIDR("0.0.0.0/32")
	_, ipNetMax, _ := net.ParseCIDR("0.0.0.1/32")
	ipr := NewRange(ipNet0)
	ipr.JoinIPNet(ipNetMax)
	ipNets := ipr.IPNets()
	ast.Equal(1, len(ipNets))
	ast.Equal("0.0.0.0/31", ipNets[0].String())

	ipr.End = net.ParseIP("255.255.255.255")
	ipNets = ipr.IPNets()
	ast.Equal(1, len(ipNets))
	ast.Equal("0.0.0.0/0", ipNets[0].String())
}
...
func TestPrefixSameLength(t *testing.T) {
	ast := assert.New(t)
	a := net.ParseIP("0.0.0.1").To4()
	b := net.ParseIP("0.0.0.0").To4()
	ast.Equal(31, PrefixSameLength(a, b))

	a = net.ParseIP("1.0.0.1").To4()
	b = net.ParseIP("2.0.0.0").To4()
	ast.Equal(6, PrefixSameLength(a, b))
}
